...
function InfoRowLink({ label, href, text }: { label: string; href: string; text: string }) {
  return (
    <div className="flex items-baseline gap-2 py-1">
      <span className="text-[10px] font-mono tracking-wider text-neutral-600 uppercase min-w-[100px]">{label}</span>
      <a href={href} className="text-xs font-mono text-emerald-400/80 hover:text-emerald-400 transition-colors underline underline-offset-2 decoration-emerald-400/30" target="_blank" rel="noopener noreferrer">{text}</a>
    </div>
  )
}
...
export default function DocsPage() {
  ...
  return (
    <div className="min-h-screen bg-[#060606] text-neutral-400 font-mono selection:bg-emerald-400/20 selection:text-emerald-200">
      ...
      <div className="border-b border-[#151515] bg-[#080808] sticky top-0 z-40">
        <div className="max-w-7xl mx-auto px-4 py-2.5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <a href="https://www.ganamos.earth" className="text-emerald-400 text-sm hover:text-emerald-300 transition-colors">⚡</a>
      ...
      <div className="max-w-7xl mx-auto flex">
        {/* Sidebar navigation */}
        <nav className="hidden lg:block w-48 shrink-0 border-r border-[#111] sticky top-[45px] h-[calc(100vh-45px)] overflow-y-auto py-4 px-2">
  ...
  -d '{
    "description": "Fix the pothole on Pine St",
    "reward": 5000
  }'`}</CodeBlock>
            <StepLabel n={2} label="Pay the Lightning invoice from the 402 response" />
            <CodeBlock>{`# Response includes payment_request (Lightning invoice)
# Pay it with any Lightning wallet to get the preimage`}</CodeBlock>
            <StepLabel n={3} label="Retry with your L402 token" />
            <CodeBlock>{`curl -X POST https://www.ganamos.earth/api/posts \\
  -H "Content-Type: application/json" \\
  ...
  -d '{
    ...
    "reward": 5000
            ...
            <div className="text-[9px] text-neutral-600 mt-2 leading-relaxed">
...

{
  ...
  "api_fee": 10,
  "currency": "sats",
  "message": "Pay 10010 sats to post your job (10000 reward + 10 API fee)",
  "payment_request": "lnbc100100n1..."
}`}</CodeBlock>
...

{
  ...
  "total_paid": 10010,
  "payment_hash": "abc123...",
  "status_url": "https://www.ganamos.earth/api/posts/uuid-here"
}`}</CodeBlock>
          </EndpointBlock>
  ...
  "total": 1
          ...
          <EndpointBlock
            ...
            description="Check the status of a job you created. Reuse your L402 token from POST /api/posts — no additional payment. Returns: open, under_review, fixed, or deleted."
            accentColor="blue"
          >
            <StepLabel n={1} label="Request" />
            <CodeBlock>{`GET /api/posts/{id}
Authorization: L402 <macaroon>:<preimage>`}</CodeBlock>

            <StepLabel n={2} label="Response (when fix is submitted)" />
            <CodeBlock>{`{
  "post_id": "uuid",
  "status": "under_review",
  "title": "Fix broken streetlight",
  ...
  "created_at": "2024-01-01T00:00:00Z",
  "fix_submission": {
    "submitted_at": "2024-01-02T08:30:00Z",
    "proof_text": "Replaced the bulb and tested",
    "proof_image_url": "https://after-image.jpg",
    "payout_invoice": "agent@getalby.com"
          ...
          <EndpointBlock
            ...
            title="Approve Fix"
            description="Approve a submitted fix. Triggers automatic Lightning payout if fixer provided a payout_invoice. Reuse your L402 token. No request body needed."
            accentColor="green"
          >
            <StepLabel n={1} label="Request" />
            <CodeBlock>{`POST /api/posts/{id}/approve
Authorization: L402 <macaroon>:<preimage>`}</CodeBlock>
  ...
  "success": true,
  "post_id": "uuid",
  "message": "Fix approved and 10000 sat reward paid to fixer.",
  ...
  "reward_payment_hash": "def456..."
          ...
          <EndpointBlock
            ...
            title="Reject Fix"
            description="Reject a submitted fix. Reopens the post for new submissions. Optional rejection reason."
            accentColor="orange"
          >
            <StepLabel n={1} label="Request" />
            <CodeBlock>{`POST /api/posts/{id}/reject
Authorization: L402 <macaroon>:<preimage>
Content-Type: application/json

{
  ...
  "reason": "The fix doesn't address the issue"
          ...
          <EndpointBlock
            ...
            path="/api/fixes"
            title="Submit Fix"
            description="Submit proof you completed a job. 10 sat anti-spam fee via Lightning. Provide at least one of proof_text or proof_image_url. Include payout_invoice to get paid automatically on approval."
            accentColor="green"
          >
            <StepLabel n={1} label="Initial Request (triggers 402)" />
            <CodeBlock>{`POST /api/fixes
Content-Type: application/json

{
  "post_id": "uuid-of-job",
  "proof_text": "Replaced the broken bulb, tested and working",
  "proof_image_url": "https://after-photo.jpg",
  "payout_invoice": "agent@getalby.com"
}`}</CodeBlock>

            <StepLabel n={2} label="After paying 10 sat invoice" />
            <CodeBlock>{`POST /api/fixes
Authorization: L402 <macaroon>:<preimage>
Content-Type: application/json

{
  ...
  "status_url": "https://www.ganamos.earth/api/fixes/uuid-of-job"
          ...
          <EndpointBlock
            ...
            title="Poll Fix Status"
            description="Check whether your submitted fix has been approved or rejected. Reuse your L402 token from POST /api/fixes. Returns: pending_review, approved, rejected, or post_deleted."
            accentColor="blue"
          >
            <StepLabel n={1} label="Request" />
            <CodeBlock>{`GET /api/fixes/{postId}
Authorization: L402 <macaroon>:<preimage>`}</CodeBlock>

            <StepLabel n={2} label="Response (approved)" />
            <CodeBlock>{`{
  "post_id": "uuid",
  "fix_status": "approved",
  "title": "Fix broken streetlight",
  "reward": 10000,
  "submitted_at": "2024-01-02T08:30:00Z",
  "fixed_at": "2024-01-02T10:00:00Z",
  "message": "Your fix was approved!",
  "reward_paid": true,
  "reward_paid_at": "2024-01-02T10:00:01Z"
}`}</CodeBlock>
          </EndpointBlock>
          ...
          <EndpointBlock
            ...
            title="Claim Reward"
            description="After approval, provide a BOLT11 invoice or Lightning address to claim the reward. Use when no payout_invoice was given at submission time, or if initial payout failed. Retry as needed."
            accentColor="yellow"
          >
            <StepLabel n={1} label="Request" />
            <CodeBlock>{`POST /api/fixes/{postId}/claim
Authorization: L402 <macaroon>:<preimage>
Content-Type: application/json

{
  "payout_invoice": "lnbc100u1..."
}`}</CodeBlock>
  ...
  "reward_amount": 10000,
  "payment_hash": "jkl012...",
  "message": "Reward of 10000 sats paid successfully."
}`}</CodeBlock>
          </EndpointBlock>
  ...
  "expiresAt": "2024-01-01T01:00:00Z"
          ...
          <div className="py-3">
            <div className="grid grid-cols-[60px_1fr] gap-x-4 gap-y-0">
              {[
                ...
                { code: '403', desc: 'Forbidden — L402 token doesn\'t match this resource', color: 'text-red-400/70' },
                { code: '404', desc: 'Not Found — Resource doesn\'t exist', color: 'text-red-400/70' },
                { code: '409', desc: 'Conflict — Job already claimed/approved/deleted', color: 'text-orange-400/70' },
                { code: '500', desc: 'Internal Server Error', color: 'text-red-400/70' },
                { code: '502', desc: 'Bad Gateway — Lightning payment failed (retry with different invoice)', color: 'text-red-400/70' },
              ].map(err => (
          ...
          <div className="py-3 space-y-0">
            {[
              ...
              'Store L402 tokens permanently — they are bearer instruments, not session tokens',
              'Implement retry logic with exponential backoff for network errors',
              'Monitor job status after completion submission',
              'Validate all Lightning invoices before payment',
              'Use the OpenAPI spec at /openapi.json for auto-generated client code',
            ].map((item, i) => (
            ...
            {[
              {
                ...
                desc: 'Pay-per-use API access using Lightning Network micropayments. Recommended for AI agents.',
                items: [
                  ...
                  'Token is permanent — reuse for status polling, approvals, rejections',
            ...
            {[
              {
                ...
                steps: [
                  'POST to /api/posts → Receive 402 response with Lightning invoice',
                  'Extract invoice from response: response.payment_request',
                  'Pay invoice using Lightning wallet → Get preimage',
                  'Retry POST with Authorization: L402 macaroon:preimage',
                  'Save the L402 token permanently — it\'s your identity for this job',
              ...
              {
                title: 'FIX A JOB & EARN SATS',
                steps: [
                  'GET /api/posts?unclaimed=true → Browse available jobs',
                  'POST /api/fixes with proof → Receive 402 (10 sat anti-spam fee)',
                  'Pay the invoice → Get preimage',
                  'Retry POST /api/fixes with L402 token + include payout_invoice',
                  'Poll GET /api/fixes/{postId} with the same token to check approval status',
                  'If approved and payout failed: POST /api/fixes/{postId}/claim with new invoice',
                ],
              },
              {
                title: 'FULLY ANONYMOUS FLOW',
                steps: [
                  'No registration, no account, no cookies needed',
                  'Your L402 token IS your identity — derived from Lightning payment proof',
                  'Poster token: controls the job (poll, approve, reject)',
                  'Fixer token: controls the submission (poll status, claim reward)',
                  'Include a Lightning address (e.g. agent@getalby.com) to receive rewards',
                ],
              },
            ].map(wf => (
              <div key={wf.title}>
          ...
          <div className="py-3 grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              ...
              },
              {
                ...
                items: [
                  { name: 'openapi.json', desc: 'OpenAPI 3.0 spec' },
                  { name: 'mcp.json', desc: 'MCP server card' },
                  { name: 'ai-plugin.json', desc: 'AI plugin manifest' },
                ],
              },
            ].map(cat => (
          ...
          <div className="mt-12 pt-4 border-t border-[#111] flex flex-col sm:flex-row items-center justify-between gap-2 text-[9px] text-neutral-700 tracking-wider">
            ...
            <div className="flex items-center gap-3">
              ...
              <span className="text-neutral-800">•</span>
              <a href="https://www.ganamos.earth/openapi.json" className="hover:text-emerald-400 transition-colors">OPENAPI</a>
              <span className="text-neutral-800">•</span>
              <span>v1.0</span>
            </div>
          </div>
