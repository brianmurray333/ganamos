import { BaseWavelengthClient, RUNTIME_MANIFEST_VERSION, WavelengthError, validateRuntimeConfig, } from '@lightninglabs/wavelength-core';
import { defaultWorkerRuntimeBaseUrl, isRuntimeAssetMessage, } from "../runtime.js";
...
import { errorMessage, toWavelengthEvent } from "../util.js";
...
function workerErrorCode(message, method) {
...
export class WorkerWavelengthClient extends BaseWavelengthClient {
    ...
    spawnWorker() {
        ...
        worker.onerror = (event) => {
            ...
            if (this.killWorker(worker, new WavelengthError(event.message || 'Wavelength worker error', 'worker_error'))) {
    ...
    replaceWorker() {
        ...
        // pending forever.
        this.rejectAll(new WavelengthError('Wavelength runtime was replaced after it exited', 'worker_error'));
    ...
    async startLocked(config) {
        ...
        if (this.disposed) {
            this.killWorker(this.worker, new WavelengthError('Wavelength client disposed', 'worker_error'));
        ...
        if (this.runtimeExited) {
            await this.lock.releaseAndSettle(this.lease);
            throw new WavelengthError('Wavelength runtime exited during start', 'worker_error');
        }
        try {
            ...
            const killed = this.killWorker(this.worker, err instanceof WavelengthError
                ? err
    ...
    request(method, params = {}) {
        ...
        if (this.runtimeExited || this.disposed) {
            return Promise.reject(new WavelengthError(this.disposed
                ? 'Wavelength client disposed'
                : 'Wavelength runtime has exited; call start() to boot a new one', 'worker_error'));
        }
    ...
    handleMessage(message, sourceWorker) {
        ...
        if (data.fatal) {
            ...
            if (this.killWorker(deadWorker, new WavelengthError(message, isWalletLockedMessage(message) ? 'wallet_locked' : 'worker_error'))) {
        ...
        pending.reject(new WavelengthError(error, workerErrorCode(error, pending.method)));
    }
    ...
    dispose() {
        ...
        if (this.killWorker(this.worker, new WavelengthError('Wavelength client disposed', 'worker_error'))) {
