import { BaseWavelengthClient, WavelengthError, validateRuntimeConfig, } from '@lightninglabs/wavelength-core';
import { RUNTIME_ASSETS } from "../runtime-manifest.js";
import { instantiateWasm, loadScript, resolveRuntimeAsset, waitForReadyEvent, wavewalletdkCall, } from "../runtime.js";
import { RuntimeLock, NO_RUNTIME_LEASE, isNearMissLockMessage, isWalletLockedMessage, } from "../runtime-lock.js";
import { debugTs, errorMessage } from "../util.js";
...
export class MainThreadWavelengthClient extends BaseWavelengthClient {
    serverTransport = 'rest';
    loadPromise = null;
    activityHandle = null;
    activityOpen = null;
    activityGeneration = 0;
    lock = new RuntimeLock({
        onWarn: (message) => this.emit({ type: 'log', payload: { level: 'warn', message } }),
    });
    ...
    onPerformance;
    runtimeCache;
    onRuntimeReady = () => this.emit({ type: 'runtimeReady' });
    constructor(options = {}) {
        super();
    ...
    async startLocked(config) {
        ...
        if (this.runtimeExited) {
            throw new WavelengthError('The Wavelength main-thread runtime has exited and cannot be ' +
                'restarted in this page; reload the page to start again', 'runtime_not_ready');
        }
        ...
        if (this.runtimeExited) {
            await this.lock.releaseAndSettle(this.lease);
            throw new WavelengthError('The Wavelength main-thread runtime exited during start', 'runtime_not_ready');
        }
        try {
            ...
            if (typeof wavewalletdkCall() !== 'function') {
                ...
                await super.stop().catch((stopErr) => {
                    ...
                    this.emit({
                        ...
                        payload: {
                            level: 'error',
                            message: 'the wallet runtime lock is retained because a recovery stop ' +
    ...
    // frees nothing.
    async afterDaemonStopped(token) {
        await this.lock.releaseAndSettle(token);
    }
    async invokeFacade(method, params = {}) {
        await this.ensureLoaded();
        const globalWallet = globalThis;
        if (typeof globalWallet.wavewalletdkCall !== 'function') {
            throw new WavelengthError('Wavelength wasm runtime is not ready', 'runtime_not_ready');
        }
        try {
            if (this.debug) {
            ...
            throw new WavelengthError(message, 
    ...
    logNearMissLock(message) {
        if (!isNearMissLockMessage(message)) {
            return;
        }
        const warning = 'a runtime failure mentioned storage or locking but was not ' +
    ...
    async pumpActivity(handle) {
        try {
            for (let entry = await handle.next(); entry !== null && this.activityHandle === handle; entry = await handle.next()) {
                this.emit({
                    type: 'activity',
    ...
    async loadRuntime() {
        ...
        const go = new goCtor();
        const result = await instantiateWasm(go.importObject, base, this.onPerformance, this.runtimeCache);
        ...
        const bootExit = runPromise.then(() => {
            throw new WavelengthError('Wavelength runtime exited before signaling ready', 'runtime_not_ready');
        }, (err) => {
            const message = errorMessage(err);
            throw new WavelengthError(message, isWalletLockedMessage(message) ? 'wallet_locked' : 'runtime_not_ready', { cause: err });
        });
