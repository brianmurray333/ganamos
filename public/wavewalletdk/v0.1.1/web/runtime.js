import { errorMessage, WavelengthError, } from '@lightninglabs/wavelength-core';
import { performanceNow, reportPerformance } from "./performance.js";
import { evictRuntimeAsset, matchRuntimeAsset, openRuntimeCache, storeRuntimeAsset, } from "./runtime-cache.js";
import { RUNTIME_ASSETS } from "./runtime-manifest.js";
/**
 ...
 */
export function resolveRuntimeAsset(base, name) {
    if (!base) {
        return name;
    }
 ...
 */
export function runtimeAssetError(url) {
    return new WavelengthError(`Wavelength runtime asset could not be loaded from ${url}. Host the daemon ` +
        'runtime assets (RUNTIME_ASSET_FILES) and point runtimeBaseUrl at them.', 'asset_load_failed');
}
/**
 ...
 */
export function isRuntimeAssetMessage(message) {
...
export function wavewalletdkCall() {
    return globalThis.wavewalletdkCall;
}
 ...
 */
async function instantiateCachedWasm(cache, url, path, importObject, onPerformance) {
 ...
 */
export async function instantiateRuntimeAsset(url, path, importObject, onPerformance, runtimeCache = true) {
    ...
    if (cache) {
        const cached = await instantiateCachedWasm(cache, url, path, importObject, onPerformance);
        if (cached) {
            return cached;
        }
    }
    ...
    if (!response.body) {
        throw new WavelengthError(`Wavelength runtime asset at ${url} arrived with no body.`, 'asset_load_failed');
    }
    ...
    if (gzipped && !('DecompressionStream' in globalThis)) {
        throw new WavelengthError(`Wavelength runtime asset at ${url} is gzip and this browser has no ` +
            'DecompressionStream to inflate it.', 'asset_load_failed');
    }
    const body = gzipped
    ...
    const compileStartedAt = onPerformance ? performanceNow() : undefined;
    const instantiated = await WebAssembly.instantiateStreaming(new Response(source, { headers: { 'content-type': 'application/wasm' } }), importObject);
    ...
    if (cache && collected) {
        void collected.then((bytes) => storeRuntimeAsset(cache, url, new Response(bytes)), () => undefined);
    }
    return instantiated;
}
/**
 ...
 */
export async function instantiateWasm(importObject, base, onPerformance, runtimeCache = true) {
    ...
    try {
        try {
            return await instantiateRuntimeAsset(resolveRuntimeAsset(base, RUNTIME_ASSETS.wasmGz), 'gzip', importObject, onPerformance, runtimeCache);
        }
        catch (err) {
            console.warn(`compressed wasm load failed: ${errorMessage(err)}`);
            path = 'raw';
        }
        return await instantiateRuntimeAsset(resolveRuntimeAsset(base, RUNTIME_ASSETS.wasm), 'raw', importObject, onPerformance, runtimeCache);
    }
    catch (err) {
        outcome = 'error';
        throw err;
 ...
 */
export function defaultWorkerRuntimeBaseUrl() {
