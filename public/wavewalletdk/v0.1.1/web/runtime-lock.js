...
const NO_WEB_LOCKS_WARNING = 'This browser has no Web Locks API, so Wavelength cannot detect the ' +
...
function warnNoWebLocks(onWarn) {
    if (warnedNoWebLocks) {
        return;
    }
 ...
 */
export function isWalletLockedMessage(message) {
    return /database is locked|SQLITE_BUSY|Access Handles cannot be created/i.test(message);
}
/**
 ...
 */
export function isNearMissLockMessage(message) {
    return (!isWalletLockedMessage(message) &&
...
export const NO_RUNTIME_LEASE = 0;
/**
...
export class RuntimeLock {
    ...
    #request(locks) {
        ...
        const granted = locks.request(RUNTIME_LOCK_NAME, { ifAvailable: true }, (lock) => {
            if (!lock) {
                this.#state = { kind: 'idle' };
                rejectLease(new WavelengthError('The wallet is already open in another tab or window. Close ' +
                    'it there and try again.', 'wallet_locked'));
                return;
            }
        ...
        void granted.catch((err) => {
            if (this.#state.kind === 'acquiring' && this.#state.lease === lease) {
                this.#state = { kind: 'idle' };
                rejectLease(new WavelengthError(`The wallet runtime lock could not be acquired: ${errorMessage(err)}`, 'runtime_lock_unavailable', { cause: err }));
            }
        });
        return promise;
    }
    ...
    async releaseAndSettle(lease) {
        ...
        const settle = Promise.resolve(granted).then(() => undefined, (err) => {
            const message = 'the wallet runtime lock release did not settle cleanly; another ' +
