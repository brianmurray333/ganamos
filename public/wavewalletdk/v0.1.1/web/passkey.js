...
async function rawSupportsPasskeyPrf() {
 ...
 */
export async function supportsPasskeyPrf() {
...
export async function registerPasskeyWallet(appName, onPerformance) {
    const salt = prfSalt();
    ...
    const createResults = created.getClientExtensionResults();
...
// wallet.
export async function assertPasskeyPrf(allowCredentialId, onPerformance) {
    const salt = prfSalt();
    const allowCredentials = allowCredentialId
        ? [{ type: "public-key", id: base64UrlToBuffer(allowCredentialId) }]
        : [];
    let assertion;
    const ceremonyStartedAt = onPerformance ? performanceNow() : undefined;
    let ceremonyOutcome = "success";
 ...
 */
export function createWebPasskeyCeremony(options = {}) {
    return {
        supportsPasskeyPrf,
        registerPasskeyWallet: (appName) => registerPasskeyWallet(appName, options.onPerformance),
        assertPasskeyPrf: (allowCredentialId) => assertPasskeyPrf(allowCredentialId, options.onPerformance),
    };
}
