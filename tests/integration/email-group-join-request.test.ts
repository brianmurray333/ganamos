...
vi.mock('@/lib/transaction-emails', () => ({
  ...
  sendGroupJoinRequestConfirmationEmail: vi.fn(async (params) => {
    const { mockEmailStore } = await import('@/lib/mock-email-store')
    mockEmailStore.storeEmail({
      type: 'group_join_request_confirmation',
      to: params.toEmail,
      subject: `You requested to join ${params.groupName}`,
      html: '', // Mock HTML content
      metadata: {
        requesterName: params.requesterName,
        groupName: params.groupName,
...
async function cleanupGroup(groupId: string) {
  ...
  try {
    // Delete activities referencing this group first (FK constraint with profiles)
    await client.query(`DELETE FROM activities WHERE related_id = $1::uuid AND related_table = 'groups'`, [groupId])
