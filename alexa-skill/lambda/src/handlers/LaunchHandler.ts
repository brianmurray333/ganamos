  ...
  async handle(handlerInput: HandlerInput): Promise<Response> {
    ...
    if (!accessToken) {
      return handlerInput.responseBuilder
        .speak(
          'Welcome to Ganamos! To get started, please link your Ganamos account in the Alexa app.'
        )
        .withLinkAccountCard()
        .getResponse();
    }
    ...
    try {
      ...
      const [balanceData, jobsData] = await Promise.all([
        ganamosClient.getBalance(),
        ganamosClient.getJobs(),
      ]);
      ...
      greeting += `You have ${speakSats(balance)}. `;
      ...
      return handlerInput.responseBuilder
        .speak(
          "Welcome to Ganamos! I'm having trouble connecting right now. Please try again in a moment."
        )
        .getResponse();
    }
  },
