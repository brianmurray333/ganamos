...
import { ganamosClient, Job } from '../api/ganamos-client';
import { speakSats, speakJob, speakDate, pause } from '../utils/speech';

/**
 * Find matching jobs based on spoken description
 */
function findMatchingJobs(jobs: Job[], spokenDescription: string): Job[] {
...
export const CompleteJobHandler: RequestHandler = {
  ...
  async handle(handlerInput: HandlerInput): Promise<Response> {
    ...
    if (!jobDescription) {
      ...
      try {
        const { jobs } = await ganamosClient.getJobs();
        ...
        if (jobs.length === 1) {
          ...
          return handlerInput.responseBuilder
            .speak(
              `I found the job "${job.title}" for ${speakSats(job.reward)}. ` +
    ...
    try {
      ...
      if (matchingJobs.length === 0) {
        return handlerInput.responseBuilder
          .speak(
            `I couldn't find an open job matching "${jobDescription}". ` +
      ...
      let speech = `I found ${matchingJobs.length} jobs that could match. ${pause()} `;

      // Describe up to 2 jobs for disambiguation
      if (matchingJobs.length >= 2) {
        const job1 = matchingJobs[0];
        const job2 = matchingJobs[1];
        speech += `The first one is "${job1.title}" for ${speakSats(job1.reward)}, ` +
                  `created ${speakDate(job1.createdAt)}. ${pause()} `;
        speech += `The second one is "${job2.title}" for ${speakSats(job2.reward)}, ` +
                  `created ${speakDate(job2.createdAt)}. ${pause()} `;
 ...
 */
export const CompleteJobDisambiguationHandler: RequestHandler = {
  canHandle(handlerInput: HandlerInput): boolean {
    const sessionAttributes = handlerInput.attributesManager.getSessionAttributes();
    const request = handlerInput.requestEnvelope.request;

    if (request.type !== 'IntentRequest') return false;
    if (!sessionAttributes.disambiguationJobs) return false;

    // Handle ordinal responses
    return (
      request.intent.name === 'SelectOptionIntent' ||
  ...
  async handle(handlerInput: HandlerInput): Promise<Response> {
    const sessionAttributes = handlerInput.attributesManager.getSessionAttributes();
    const jobs = sessionAttributes.disambiguationJobs as Job[];
    ...
    return handlerInput.responseBuilder
      .speak(
        `Okay, "${job.title}" for ${speakSats(job.reward)}. ` +
        `Should I mark it as complete and assign it to ${fixerName}?`
      )
 ...
 */
export const CompleteJobConfirmHandler: RequestHandler = {
  canHandle(handlerInput: HandlerInput): boolean {
    const sessionAttributes = handlerInput.attributesManager.getSessionAttributes();
    return (
      handlerInput.requestEnvelope.request.type === 'IntentRequest' &&
  ...
  async handle(handlerInput: HandlerInput): Promise<Response> {
    ...
    try {
      const result = await ganamosClient.completeJob(jobId, fixerName);
      ...
      return handlerInput.responseBuilder
        .speak(result.message ||
          ...
          `${fixerName} has been awarded ${speakSats(reward)}.`)
        .getResponse();
    } catch (error) {
      console.error('Complete job error:', error);
      return handlerInput.responseBuilder
 ...
 */
export const CompleteJobCancelHandler: RequestHandler = {
  canHandle(handlerInput: HandlerInput): boolean {
    const sessionAttributes = handlerInput.attributesManager.getSessionAttributes();
    return (
      handlerInput.requestEnvelope.request.type === 'IntentRequest' &&
